...
export default defineComponent({
    ...
    props: {
        ...
        onSubmit: {
            type: Function as PropType<(values: Omit<Script, 'id'>, resetFields: (newValues?: Props | undefined) => void) => void>,
            required: true
        }
    },
    components: {
    ...
    setup(props): CreateFormSetupData {
        ...
        // 表单值
        const modelRef = reactive<Omit<Script, 'id'>>({
            name: '',
            desc: '',
        });
        // 表单验证
        ...
        const onFinish = async () => {           
            try {
                const fieldsValue = await validate<Omit<Script, 'id'>>();
