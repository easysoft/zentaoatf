...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if serverConsts.CONFIG.Mysql.LogZap != "" {
		...
		case 1:
			logUtils.Infof("gorm", zap.Any("src", data[0]))
		case 2:
			logUtils.Infof("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			logUtils.Infof("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			logUtils.Infof("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {
