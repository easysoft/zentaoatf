...
func Init() {
	config := consts.ConfigFileServer
	fmt.Printf("配置文件路径为%s\n", config)

	v := viper.New()
	serverConsts.VIPER = v
	serverConsts.VIPER.SetConfigType("yaml")
...
zap:
 ...
 log-in-console: true`)
		...
		if err := serverConsts.VIPER.ReadConfig(bytes.NewBuffer(yamlDefault)); err != nil {
		...
		if err := serverConsts.VIPER.WriteConfigAs(config); err != nil {
	...
	// 存在配置文件，读取配置文件内容
	serverConsts.VIPER.SetConfigFile(config)
	err := serverConsts.VIPER.ReadInConfig()
	if err != nil {
